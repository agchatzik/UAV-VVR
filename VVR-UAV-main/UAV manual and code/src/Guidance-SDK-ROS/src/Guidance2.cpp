...
{
   myfile1.open ("/home/ubuntu/Desktop/imu_measurements/imu2.txt", std::ofstream::out | std::ofstream::app);
    myfile2.open ("/home/ubuntu/Desktop/imu_measurements/velocity2.txt", std::ofstream::out | std::ofstream::app);
    /// image counter raize
    image_counter ++;
    g_lock.enter();
    ...
    {        
		...
		if ( data->m_greyscale_image_left[CAMERA_ID] ){
			...
			//if (show_images) {
			  //  cv::namedWindow("left");
			   // cv::imshow("left", g_greyscale_image_left);
			//}
            ...
            image_distance << "/home/ubuntu/Desktop/Drone_stereo_Images/images_left_2/Scene"<<image_counter<<".jpg";
		...
		if ( data->m_greyscale_image_right[CAMERA_ID] ){
            ...
            std::ostringstream image_distance;
            image_distance << "/home/ubuntu/Desktop/Drone_stereo_Images/images_right_2/Scene"<<image_counter<<".jpg";
            std::string copyOfStr = image_distance.str();
...
//    /* imu */
    if ( e_imu == data_type && NULL != content )
   {
     ...
     //   }
            ...
            myfile1 <<fixed<< std::setprecision(4) << imu_data->q[0]<< "\t"<< imu_data->q[1]<< "\t"<< imu_data->q[2]<< "\t"<< imu_data->q[3]<<endl;
    }
//    /* velocity */
    if ( e_velocity == data_type && NULL != content )
    {
       ...
       // }
      myfile2<<fixed<< std::setprecision(4) <<  0.001f * vo->vx << "\t"<< 0.001f * vo->vy << "\t"<< 0.001f * vo->vz <<endl;
...
//    disp_image_pub			= my_node.advertise<sensor_msgs::Image>("/guidance/disparity_image",1);
//
